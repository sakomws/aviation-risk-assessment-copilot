    ...
    async def start(self):
        ...
        self.ctx.create_task(
            self.chat.send_message(
                "Welcome to the SRE Copilot Assistant! Speak when you are ready to fly."
            )
        )
    ...
    async def image_generation_worker(self):
        ...
        while True:
            ...
            if prompt:
                ...
                self.ctx.create_task(
                    self.chat.send_message(
                        f'Loading SRE for: "{prompt}"'
                    )
                )
                started_at = datetime.now()
                try:
                   argb_frame = await self.dalle.generate("Show Plane", size="1792x1024")
                   argb_frame2 = await self.dalle.generate("Show Plane inside", size="1792x1024")
                   self.current_image = argb_frame
                   # elapsed = (datetime.now() - started_at).seconds
                   reply=demo_inference.generate_risk(prompt)
                   print(reply)
                   self.ctx.create_task(
